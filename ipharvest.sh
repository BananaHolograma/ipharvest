...
EXAMPLES:
    ...
    ipsoak -s https://example.com/log.txt --geolocation
    ipsoak -s ipv6.txt --mode ipv6 -o documents/reports/ipv6.json
    ipsoak -source /var/log/example.log --geolocation --mode both -o reports/ip_harvest.csv
...
OPTIONS:
    ...
    -o  --output                      Define a file path to save the report generated by the tool (can be json or csv)
